...
///TODO: REWRITE TESTS USING FUNCTIONS
void testIndexCreation()
{
    std::cout << "Creating an Index File" << std::endl;
    try{
        BTreeIndex index("test", intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
    }catch(HeaderPageNumberIncorrect e){
        std::cout << "Test Index File creation Failed" << std::endl;
        return;
    }catch(RootPageNumberIncorrect e){
        std::cout << "Test Index File creation Failed" << std::endl;
        return;
    }
    std::cout << "Test Index File created with Meta Page and Empty Root Node" << std::endl;

}

void testIndexOpen()
{
    std::cout << "Opening Already Created Index FIle" << std::endl;
    try {
        BTreeIndex index("test", intIndexName, bufMgr, offsetof(tuple, i), INTEGER);
    }catch(RelationNameIncorrect e){
        std::cout << "Test Index File open Failed" << std::endl;
        return;
    }
    std::cout << "Test Index File opened and Relation name verified" << std::endl;
    File::remove("test.0");
}
...
void testRootFill()
{
    ...
    std::cout << root << ": Should be like 3?" << std::endl;
